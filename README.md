...
현재 영상 빌더(t2v·i2v)는 Veo · **Seedance 2.5** · Seedance 2.0 · 범용 셋을 지원한다.

### Seedance 2.5 / 2.0 모드

Seedance 계열은 카메라·렌즈 키워드보다 **"언제 무엇이 바뀌는가"**를 분명히 적는 것이 중요하다.
Higgsfield에서 제공하는 2.5의 최대 30초 흐름과 2.0의 기존 6/10초 연속숏 형식을 각각
지원한다. 저장된 2.0 작업의 결과가 자동으로 바뀌지 않도록 모델 키와 타임라인을 분리했다.
두 모델 모두 전용 입력 패널을 쓴다 (`CONFIG.sd` 가 있는 앱에서 엔진이 만든다. 이미지 빌더에는 없다).
...
> 참고: 기존 Seedance 2.0 저장 작업은 `seedance`, Seedance 2.5는 `seedance25` 키로
> 따로 복원한다. 이미지 빌더의 구형 `nano`·`ideogram` 키는 `natural`로 명시적으로 이전한다.
