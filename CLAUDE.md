...
## 2-1. 아직 남아 있는 것 (2026-08-07 기준)

**레퍼런스 룩·직접 선택 사진 71장의 원본이 640~680px 밖에 안 된다.** 가이드 사진 20장은
1024px 로 올려서 해결했지만 이쪽은 손대지 못했다. 코드로 고칠 수 있는 문제가 아니다 —
원본보다 큰 그림은 만들 수 없다.

폭 1040px 아래에서는 옵션 카드가 1열이 되어 화면 폭만큼 커지는데, 그 구간에서만 문제가 된다.
...
즉 **휴대폰과 넓은 데스크톱은 멀쩡하고, 창을 반쯤 줄인 노트북 폭에서만 흐리다.**
고치려면 1376px 급 원본을 다시 만들어야 한다 — 가이드 이미지 때와 같은 절차다.
그때는 `06-render.js` 의 `guideImg` · `GUIDE_SIZES` 와 `verify-guide-assets.mjs` 의
`RUNGS` 를 그대로 흉내 내면 된다.

지금 파생본을 만드는 건 실익이 없다. 데스크톱 dpr2 에서 680px 은 이미 적정(1.25배)이라
줄일 여지가 없고, 줄여서 이득이 나는 건 dpr1 화면뿐이다.
