...
interface ModelSpec {
  ...
  /**
   * 이 앱이 권장하는 길이(단어). **모델의 입력 한계가 아니다.**
   *
   * 지금 값은 대표 프리셋을 실제로 조립해 본 최대치에 입력 여유를 더한 것이다 —
   * 즉 **앱이 만든 출력에서 역산한 값**이라, 구조상 "이 모델에는 너무 길다" 는 말을
   * 할 수 없다. 재고 있는 것은 "우리 프리셋보다 긴가" 뿐이다.
   * 모델마다의 실제 입력 한계는 아직 조사하지 않았다(특히 SDXL 계열 텍스트 인코더).
   * 그 값이 생기기 전까지 이 숫자를 모델 한계처럼 쓰거나 설명하지 말 것.
   */
