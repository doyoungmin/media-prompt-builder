...
/* ── 라벨 → 사진 ──
   세 앱에 같은 내용이 세 벌 복사돼 있었다. 같은 라벨이 앱마다 다른 사진을 가리켜도
   당시 검수는 파일 목록만 대조해서 통과했다 — 그래서 정본을 여기 하나로 모은다.
   사진을 쓰지 않는 앱은 CONFIG.noPreviewImages 로 끈다(05-derive 의 usePhotos). */
const PHOTO = {
